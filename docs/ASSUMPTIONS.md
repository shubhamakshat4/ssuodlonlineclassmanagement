...
## Change 24 Sep 2026 — real ODL data, programme+semester class groups
- A `batches` row now means a **class group = programme + semester** (`BBA-S1`, `MHS-S2`), because the real timetable is organised that way, not by intake cohort. The cohort the student was admitted in moves to `students.intake_session` (plus `enrollment_no`, `personal_email`). — Matches the source spreadsheets and the owner's instruction; avoided a wholesale table rename, so RLS/views/jobs kept working. — Medium: renaming `batches` → `class_groups` later is cosmetic but touches many files.
- `students.secondary_batch_id` is the optional second class group (a semester being caught up on). `current_student_batch_ids()` returns both, and every policy/helper that scoped on one batch now accepts either. — Owner's requirement (max two). — Low.
- Semester is derived from the admission session for the Sep–Dec 2026 term: Feb-2025 → S4, Aug-2025 → S3, Feb-2026 → S2, Aug-2026 → S1. This matches the four cohorts the timetable schedules per programme. — Needs revisiting each term: set the students' groups for the new term (CSV import or the Students screen). — Medium; it is data, not code.
- The real timetable is **dated**, not weekly-recurring (16 specific Sundays, different subjects each week), so rows are imported directly as `class_sessions` with `timetable_slot_id = null`. `timetable_slots` and the nightly generator stay for future recurring use. — The source data has no repeating weekly pattern per subject. — Low.
- 286 new admissions (Aug-2026) have no university email or roll number yet, so the import parks a deterministic placeholder (`first.last.tmp-<prog>-aug2026-nnnn@srisriuniversity.edu.in`, roll `TMP-…`) and keeps the personal email in `students.personal_email`. They cannot sign in until the real address replaces the placeholder. — Keeps the roster complete and visible to admins. — Low: fix by CSV import or the Students screen.
- Faculty are created from the timetable with placeholder logins (`first.last@srisriuniversity.edu.in`) and a matching `…@srisriuniversity.onmicrosoft.com` UPN, flagged `must_change_password`. — The owner will supply real addresses; co-organiser assignment is nominal until then. — Low.
- `join_window_lead_minutes` is now **15** (owner's instruction: classes longer than 30 minutes open 15 minutes early). The Join button is never greyed out before the window: pressing it explains when the class opens. The server still enforces the window in the attendance trigger. — Requested; security unchanged. — Trivial.
- Source spreadsheets in `docs/*.xlsx` are **gitignored**: they hold real names, personal emails and phone numbers for 637 students. — Data protection; the repository is on GitHub. — None.
- The admin Students list is paged (50 per page); it read all 637 rows before and took ~7.6 s. — Real cohort size. — Trivial.
